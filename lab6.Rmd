...
```
INCOMPLETE: NOT SURE WHEN THEY ARE USED!!
...
xg_learn_tuned %>% tune::show_best(metric = "mn_log_loss")
best_learn <- xg_learn_tuned %>% tune::select_best(metric = "mn_log_loss")
...
xgmodel_three <-parsnip::boost_tree(
  ...
  learn_rate = best_learn, #eta
  mtry = tune(), #colsample_bytree
  min_n = tune(), #min_child_weight
  tree_depth = tune(), #max_depth
  sample_size = tune()
  
) #DO WE NEED TO SET SOME SET_ENGINE PARAMS ??
  # set_engine("xgboost", 
  #            objective = "multi:softprob",
  #            lambda=0, 
  #            alpha=1, 
...
```{r}
xg_three_tuned %>% tune::show_best(metric = "mn_log_loss")

three_metrics <- xg_three_tuned %>% tune::select_best(metric = "mn_log_loss")
...
stochastic_model <-parsnip::boost_tree(
  ...
  mtry = three_metrics$mtry, #colsample_bytree
...
```{r}
sample_size_Params <- dials::parameters(
  sample_size(),
  finalize(mtry(), select(eel_recipe, - angaus)), #mtry() is the number of variables used in the making of each tree we need to bound it by the number of variables available.
  sample_size = sample_prop(c(0.4, 0.9))
)
...
```{r}
stochastic_grid <- dials::grid_max_entropy(sample_size_Params, size = 50) #GRID specs from the book
```
...
```{r}
stochastic_tuned %>% tune::show_best(metric = "mn_log_loss")

stochastic_metrics <- stochastic_tuned %>% tune::select_best(metric = "mn_log_loss")
